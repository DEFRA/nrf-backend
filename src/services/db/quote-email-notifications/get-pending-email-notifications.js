...
// duplicates this same list in its WHERE predicate (Postgres rewrote the
// predicate to `notify_send_status` when the column was renamed in 3.2). If you
// add a status here, add a matching changeset that rebuilds that index —
// otherwise Postgres can no longer prove the index predicate implies this
// query's predicate and the poller falls back to a full scan every tick.
 ...
 * a notification_id has been recorded (Notify-rejected retries leave it null
 * and are skipped), and status is unknown or not yet terminal, within the
 * lookback window. Ordered oldest-first and capped at `limit` so each run
 * does bounded work.
 *
 * @param {object} params
  ...
  const { rows } = await db.query(
    `SELECT id, notification_id
      ...
      WHERE notification_id IS NOT NULL
        AND (notify_send_status IS NULL OR notify_send_status NOT IN (${terminalLiterals}))
